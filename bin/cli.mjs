...
// Stop hook commands earlier releases installed. Before 2.1.0 was on npm this
// one failed in any project without the kit in node_modules, and the agent was
// told the gate had found a regression. Once it is on npm, that same project
// would fetch the newest release after every turn. `add --with-hook` replaces it.
const LEGACY_HOOK_COMMANDS = new Set(["npx ucsd-decorator-kit verify || exit 2"]);
// Each of these writes something that runs the kit from this project's
// node_modules — the workflow and scripts through `decorator:*`, the hook
// directly — so each also installs the kit as a devDependency.
const KIT_FLAGS = ["with-ci", "with-hook", "with-decorator"];
// Install-once, like the CI templates: both directories belong to the project's
// developers, so their READMEs are never in `manages` and never overwritten.
const CANVAS_RULES_README = `${CANVAS_RULES_DIR}/README.md`;
const CANVAS_COMPONENTS_README = `${CANVAS_COMPONENTS_DIR}/README.md`;
...
const KIT_SPEC = `${kitPackage.name}@${VERSION}`;
...
async function writeManifest(managed) {
  ...
  const manifest = {
    // Pinned to the version that wrote this file, so an editor validates it
...
async function scaffoldCanvasDirectories() {
  ...
  await put(CANVAS_COMPONENTS_README, await readFile(path.join(TEMPLATES, "canvas-components-README.md"), "utf8"));
}
 ...
 *
 *   - Two Stop hooks. The first runs `verify` from this project's
 *     node_modules, backgrounded via `asyncRewake` so it never blocks the turn
 *     from ending, but if it finds a regression the agent is woken back up
 *     with the findings fed back as context — the exit-code-2 translation
 *     (`|| exit 2`) is what asyncRewake keys on. It never runs `npx`, which
 *     would fetch whatever release is newest on the registry after every turn.
 *     When the kit is not installed it exits 0, and the second hook tells the
 *     user instead: a non-blocking error, shown to the person, not the agent.
...
async function installClaudeSettingsHook() {
  ...
  const commands = stopGroup.hooks.map((hook) => hook.command);
  ...
  settings.hooks ??= {};
  settings.hooks.Stop ??= [];
  const installed = new Set(settings.hooks.Stop.flatMap((group) => group.hooks ?? []).map((hook) => hook.command));
  const current = commands.every((command) => installed.has(command));
  const legacy = [...LEGACY_HOOK_COMMANDS].some((command) => installed.has(command));
  if (!current || legacy) {
    // Take out every hook this kit has ever installed, leaving the project's
    // own hooks where they are, then add the current set as one group.
    const ours = (hook) => commands.includes(hook.command) || LEGACY_HOOK_COMMANDS.has(hook.command);
    settings.hooks.Stop = settings.hooks.Stop.flatMap((group) => {
      if (!group.hooks?.some(ours)) return [group];
      const hooks = group.hooks.filter((hook) => !ours(hook));
...
/**
 * Install the kit as a devDependency, at the version doing the installing.
 *
 * The same version, not the newest: the files this run just generated carry
 * its version, and a newer kit would call them stale on the first `check`.
 * A project that already declares the kit keeps the version it chose.
 */
async function installKit() {
  const raw = await readFile(path.join(cwd, "package.json"), "utf8").catch(() => null);
  const manifest = raw === null ? {} : JSON.parse(raw);
  if (manifest.devDependencies?.[kitPackage.name] ?? manifest.dependencies?.[kitPackage.name]) return true;
  return install([KIT_SPEC]);
}

function printKitMissing() {
  console.log(`${kitPackage.name} is not installed here. The decorator:* npm scripts, the CI`);
  console.log("workflow, and the Claude Code hook all run it from node_modules, so until it");
  console.log("is installed they fail, and the hook skips the chrome gate. Install it:");
  console.log("");
  console.log(`  npm install --save-dev ${KIT_SPEC}`);
}
...
async function init() {
  ...
  console.log("");
  console.log(`Project-specific rules for the canvas go in ${CANVAS_RULES_DIR}/ as Markdown files, and`);
  console.log(`component libraries in ${CANVAS_COMPONENTS_DIR}/, one folder each with a README.md.`);
  console.log("Run `npx ucsd-decorator-kit sync` after adding either.");
}
...
async function add() {
  ...
  let kitInstalled = true;
  if (KIT_FLAGS.some(has)) {
    kitInstalled = await installKit();
    await addScripts();
  }

  // Re-running `add` on a project `init` set up must not drop AGENTS.md from
  // `manages` — `sync` would silently stop refreshing it.
  const existing = await readManifest();
  const keepsContract = existing?.manages?.includes(PROJECT_CONTRACT) ? [PROJECT_CONTRACT] : [];
  const managed = [...outputs.keys(), ...keepsContract, SKILL_DEST];
  ...
  if (!has("with-ci")) {
    ...
    console.log(`and adds ${kitPackage.name} as a devDependency for it to run.`);
  }
  if (!has("with-hook")) {
    console.log("");
...
async function check() {
  ...
  if (stale.length) {
    console.error(
      `These files are out of date with ucsd-decorator-kit@${VERSION}, ${CANVAS_RULES_DIR}/, or ${CANVAS_COMPONENTS_DIR}/:`,
    );
    for (const entry of stale) console.error(`  ${entry}`);
    console.error("");
    console.error(
      `If you upgraded the kit, changed a file in ${CANVAS_RULES_DIR}/, or added or changed a library in ${CANVAS_COMPONENTS_DIR}/, that is expected.`,
    );
...
function help() {
  ...
  npx ucsd-decorator-kit sync     refresh managed files after upgrading the kit or
                                  editing ${CANVAS_RULES_DIR}/ or ${CANVAS_COMPONENTS_DIR}/
